...
以下术语在 AWLY 的范畴内，有统一的推荐译法。

### backport → 向后移植

动词用法：`向后移植（backport）`；名词用法：`向后移植（backported）的补丁 / 变更 / …`。

不建议使用「回合」，因为「回合」在中文中更常见的义项是「轮次」（n. turn），
在 backport 语境下会产生歧义——读者可能误读为「第 N 回合提交」而非「向后移植到旧版本的提交」。

:::tip[如何记忆]

在体育、游戏等领域，「回合制」是耳熟能详的概念；反观「向后移植」这一操作，
除了音近的巧合（`backport` → `back + port`）之外，与「有人出完招了，轮到下一个」的意思完全无关。
