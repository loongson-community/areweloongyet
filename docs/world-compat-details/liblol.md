...
如果您不清楚龙架构在新世界和旧世界上的兼容性问题，可以先阅读 [旧世界与新世界](/docs/old-and-new-worlds) 一文，当然不阅读也不影响您理解下列关键结论。

### 问：使用 `libLoL` 能解决什么问题？

截止2024年2月，已经发售的龙芯3A5000和龙芯3A6000系列的龙架构办公终端类产品，使用的软件都是“旧世界”的软件。
如果您使用过这类终端，相信您很可能使用过WPS、腾讯会议、微信、QQ、炒股软件、龙芯浏览器等软件。您的公司也可能基于旧世界购买了或开发了一些商业软件。到目前为止,这些都是基于“旧世界”的软件。
未来发布的新版本操作系统（如办公用户常见的UOS、麒麟等），很可能是基于龙芯“新世界”构建的。
有了 `libLoL` 上述这些“旧世界”软件就可以直接在新的操作系统上运行，不会面临兼容性问题。就像您可以在Windows 10上以兼容模式运行为Windows 98设计的软件一样。

### 问：下面提到，旧世界输入法无法在新世界使用，会不会导致我无法输入中文？

不会，所有厂商的操作系统都会带默认的输入法。您可以使用默认的输入法输入文字。默认的输入法可以在所有的应用程序中输入文字，无论这个程序是新世界的还是旧世界的。
下面所说的旧世界输入法无法使用，是指新世界操作系统发售后，短时间内会有一些应用市场里的第三方输入法（例如搜狗拼音）可能不能正常兼容使用。
您不需要过于担心，一般在新操作系统发布后，几个月内厂商就会发布新版本实现对新系统的兼容。
在此之前，您可以先用系统默认的输入法作为过渡，也可以先用旧版本的操作系统（就好像windows 11还有各种兼容性问题的时候，您完全可以先用windows 10一样）。
