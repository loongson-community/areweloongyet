...
非技术用户可能对 `libLoL` 的具体实现不感兴趣，只想知道 `libLoL` 究竟解决了什么问题？

如果您不清楚龙架构在新世界和旧世界上的兼容性问题，可以先阅读[《旧世界与新世界》](../old-and-new-worlds.md)一文，当然这不影响您理解下列关键结论。

### 问：使用 `libLoL` 能解决什么问题？

截至 2024 年 2 月，已经发售的龙芯 3A5000 和龙芯 3A6000 系列等龙架构&reg;办公终端类产品，出厂或交付时默认搭载的都是「旧世界」的软件。
如果您使用过这类终端，相信您很可能使用过 WPS、腾讯会议、微信、QQ、炒股软件、龙芯浏览器等软件。您的公司也可能为其购买或开发了一些商业软件。到目前为止，这些都是基于「旧世界」的软件。
而未来发布的新版本操作系统（如办公用户常见的 UOS、麒麟等），则很可能是基于龙芯「新世界」构建的。
有了 `libLoL`，上述这些「旧世界」软件就可以直接在新的操作系统上运行，不会面临兼容性问题——就像您可以在 Windows 10 上以兼容模式运行为 Windows 98 设计的软件一样。

### 问：下面提到，旧世界输入法无法在新世界使用，会不会导致我无法输入中文？

不会。

所有厂商的操作系统都会带默认的输入法。您可以使用默认的输入法输入文字。默认的输入法可以在所有软件中输入文字，无论这个程序是新世界的还是旧世界的。
下面[所说的](#why-its-impossible-to-load-ow-libs-in-nw-apps)「旧世界输入法无法使用」，仅仅指新世界操作系统发售后，短时间内会有一些应用市场里暂未适配新世界的第三方输入法（如搜狗拼音）可能不能搭配新世界软件使用。
对此您不需要过于担心：一般在新操作系统发布后，几个月内应用厂商就会发布兼容新系统的升级包。
在此之后，您就可以在新系统上安装并使用您熟悉的第三方输入法了；
在此之前，您可以先用系统默认的输入法作为过渡，也可以先继续使用旧版本的操作系统（就好像 Windows 11 还有各种兼容性问题的时候，您完全可以先用 Windows 10 一样）。

## 技术特点 {#features}

`libLoL` 是由 AOSC 社区开发的兼容方案，由两部分组成：
