...
async function getCategoryMetadata(categoryDirPath: string): Promise<CategoryMetadata> {
...
async function readCategories(sourcePath: string): Promise<IProjectCategory[]> {
  ...
  for await (const dirent of srcDir) {
    ...
    const projectDefPaths = projectDefFilenames.map((x) => path.join(categoryDirPath, x))
...
export default async function awlyDataPlugin(
  ...
  return {
    ...
    getPathsToWatch() {
      return [
        path.join(options.sourcePath, '**/*.yml'),
      ]
    },
  }
}
