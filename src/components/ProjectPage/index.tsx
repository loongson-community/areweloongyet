  ...
  return (
    <section>
      <dl className={styles.infoTable}>
        ...
        <dt>适配工作质量</dt>
        <dd>
          <CodeQualityIcon val={data.quality} showDesc={true} />
        </dd>
        <dt>支持状态</dt>
        <dd>
          <SupportStatusIcon val={data.supportStatus} showDesc={true} />
        </dd>
        <dt>何时开始支持</dt>
      ...
      <main className="container">
        ...
        <section>
          ...
          <dl className={styles.infoTable}>
            ...
            <dd>
              {data.homepageURL ? (
                <a href={data.homepageURL}>{data.homepageURL}</a>
              ) : (
                '暂缺'
              )}
            </dd>
            ...
            <dd>
              {data.repoURL ? (
                <a href={data.repoURL}>{data.repoURL}</a>
              ) : (
                '暂缺'
              )}
            </dd>
        ...
        <section>
          <h3>适配工作</h3>
          {data.portingEfforts.map((x, i) => (
            <PortingEffort key={i} data={x} />
          ))}
        </section>
      </main>
    </Layout>
