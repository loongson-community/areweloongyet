...
export const InputMinifloat: React.FC<
  InputMinifloatProps & React.HTMLAttributes<HTMLDivElement>
> = (props) => {
  const [bitRepr, setBitRepr] = useState(props.bitRepr ?? 0)
  const { neg, isZero, apparentExp, unbiasedExp, mantissa, apparentMantissa } =
    props.fmt.decompose(bitRepr)

  const signalChangeEvent = (bitRepr: number) => {
    props.onMinifloatValueChange?.({
      value: props.fmt.asNumber(bitRepr),
      bitRepr,
    })
  }

  const minApparentMantissa = props.fmt.implicitMantissaOffset
  const maxApparentMantissa =
    props.fmt.maxMantissa + props.fmt.implicitMantissaOffset
  ...
  const inputStateMantissaFormatter = (inputState: number | string): string => {
    inputState =
      typeof inputState == 'string' ? parseInt(inputState, 10) : inputState
  ...
  const signSelect = (
    <Select value={neg} onChange={onToggleSign}>
  ...
  const onInputStateMantissaChange = (val: number | null) => {
    val = val ?? 0
    const newMantissa =
      toApparentMantissa(val) - props.fmt.implicitMantissaOffset
  ...
  return (
    <>
      {mantissaInput} &times; pow(2, {expInput}) {equalOrRepresentsSign}{' '}
      {numRepr}
      <br />
      迷你浮点数格式：符号 1 位 + 指数 {props.fmt.expWidth} 位 (偏置{' '}
      {props.fmt.expBias}) + 尾数 {props.fmt.mantissaWidth} 位。
      {props.fmt.supportsZero ? '支持' : '不支持'}表示零值。
      {zeroNotes}
    </>
  )
}
