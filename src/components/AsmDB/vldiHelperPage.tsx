...
const VlenSelect: React.FC<
  VlenSelectProps & React.HTMLAttributes<HTMLDivElement>
> = (props) => {
  ...
  return (
    <Radio.Group onChange={onChange} defaultValue={vlen} buttonStyle="solid">
...
const ElementTypeSelect: React.FC<
  ElementTypeSelectProps & React.HTMLAttributes<HTMLDivElement>
> = (props) => {
  ...
  return (
    <Radio.Group onChange={onChange} value={elemTy} buttonStyle="solid">
...
const availableVldiFunctionsByElemTy = [
  {
    ...
    avail: [
      ...
      {
        f: VldiFunction.BroadcastU8To8Alternate,
        desc: '取 8 位整数，广播（形式二，等价于形式一）',
      },
    ],
  },
  {
    ...
    avail: [
      {
        f: VldiFunction.BroadcastS10To16,
        desc: '符号扩展 10 位整数到 16 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8To16,
        desc: '零扩展 8 位整数到 16 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8Shl8To16,
        desc: '零扩展 8 位整数到 16 位，左移 8 位，广播',
      },
    ],
  },
  {
    ...
    avail: [
      {
        f: VldiFunction.BroadcastS10To32,
        desc: '符号扩展 10 位整数到 32 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8To32,
        desc: '零扩展 8 位整数到 32 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8Shl8To32,
        desc: '零扩展 8 位整数到 32 位，左移 8 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8Shl16To32,
        desc: '零扩展 8 位整数到 32 位，左移 16 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8Shl24To32,
        desc: '零扩展 8 位整数到 32 位，左移 24 位，广播',
      },
      {
        f: VldiFunction.BroadcastU8FFTo32,
        desc: '零扩展 8 位整数到 32 位，左移 8 位，低位以全 1 填充，广播',
      },
      {
        f: VldiFunction.BroadcastU8FFFFTo32,
        desc: '零扩展 8 位整数到 32 位，左移 16 位，低位以全 1 填充，广播',
      },
    ],
  },
  {
    ...
    avail: [
      {
        f: VldiFunction.BroadcastS10To64,
        desc: '符号扩展 10 位整数到 64 位，广播',
      },
      {
        f: VldiFunction.BroadcastBitExpandedU8To64,
        desc: '取 8 位整数，将每位重复 8 次，广播',
      },
    ],
  },
  {
    ...
    avail: [
      {
        f: VldiFunction.BroadcastVldiMinifloatToF32,
        desc: '转换迷你浮点数为 float，广播',
      },
      {
        f: VldiFunction.BroadcastVldiMinifloatToEvenF32,
        desc: '转换迷你浮点数为 float，广播到偶数编号元素，置零奇数编号元素',
      },
    ],
  },
  {
    ...
    avail: [
      {
        f: VldiFunction.BroadcastVldiMinifloatToF64,
        desc: '转换迷你浮点数为 double，广播',
      },
    ],
  },
]
...
const VldiFunctionSelect: React.FC<
  VldiFunctionSelectProps & React.HTMLAttributes<HTMLDivElement>
> = (props) => {
  const [state, setState]: [
    VldiFunctionSelectState,
    Dispatch<SetStateAction<VldiFunctionSelectState>>,
  ] = useState({
    ...
    <Card variant="borderless">
      <Row>
        <Col span={24}>
          ...
          <ElementTypeSelect
            elemTy={state.elemTy}
            onElementTypeChange={onElemTyChange}
          />
        </Col>
      </Row>
      <Row {...marginTop}>
        <Col span={24}>
          装载操作：
          <Radio.Group
            onChange={onVldiFunctionChange}
            value={state.selectedFuncByElemTy[state.elemTy]}
          >
...
export default function VldiHelperPage(): React.JSX.Element {
  ...
  return (
    <>
      <p>
        基于
        <a href="https://github.com/jiegec">@jiegec</a>的
        <a href="https://jia.je/unofficial-loongarch-intrinsics-guide">
          <i>Unofficial LoongArch Intrinsics Guide</i>
        </a>
        制作。
      </p>
        ...
        <Col span={24}>
          <Card variant="borderless">
            <Row>
              <Col span={24}>
                向量宽度：
                <VlenSelect vlen={vlen} onVlenChange={setVlen} />
                <Checkbox
            ...
            <Row {...marginTop}>
              <Col span={24}>
                <pre>
                  ...
                  <br />
                  <br />
                  {resultCType} val = ({resultCType}){intrinsicName}(
                  {vldiSImmHex});
                  <br />
                  {resultDemoCodeLines.map((line, i) => (
                    <span key={`vldiHelperPage-demo-code-l${i}`}>
                      {line}
                      <br />
                    </span>
                  ))}
