...
import {
  augmentDecodeTree,
  mapifyAugmentedDecodeTree,
  type AugmentedNodeMap,
...
const DecodeTreeView: React.FC<
  DecodeTreeViewProps & React.HTMLAttributes<HTMLDivElement>
> = (props) => {
  let onSelect = null
...
> = (props) => {
  ...
  if (!selectedNode.node) {
    ...
    const subspaceUsageRatio =
      (m.numUsedInsnWords / m.parentNode.numTotalInsnWords) * 100
    const universeUsageRatio = (m.numUsedInsnWords / 0x100000000) * 1000
    const universeUsageRatioText =
      universeUsageRatio < 0.001
        ? '< 0.001'
      ...
      <>
        ...
        <Row gutter={16}>
          <Col span={8}>
            <Statistic
              title="本级空间占比"
              value={subspaceUsageRatio.toFixed(2)}
              suffix="%"
              style={vertMargin}
            />
          </Col>
  ...
  const subspaceUsageRatio = root
    ? 1
    : (n.numTotalInsnWords / n.parentMatch.parentNode.numTotalInsnWords) * 100
  const universeUsageRatio = root
    ? 1
    : (n.numTotalInsnWords / 0x100000000) * 1000
  const universeUsageRatioText =
    universeUsageRatio < 0.001
      ? '< 0.001'
      : universeUsageRatio.toFixed(3).toString()
  ...
  let insnFmtDisplay
  if (n.possibleFmts.length == 1) insnFmtDisplay = n.possibleFmts[0]
  else if (n.possibleFmts.length < 50)
    insnFmtDisplay = (
      <>
        {_.map(n.possibleFmts, (x) => (
          <Tag className={styles.insnFmtTag}>{x}</Tag>
        ))}
      </>
    )
  else insnFmtDisplay = `${n.possibleFmts.length} 种`
    ...
    <>
      ...
      <Row gutter={16}>
        ...
        <Col span={8}>
          <Statistic
            title="已分配比例"
            value={allocationRatio.toFixed(2)}
            suffix="%"
            style={vertMargin}
          />
        </Col>
...
export default function EncodingSpaceOverviewPage({
  data,
}: {
  data: AsmDBData
}): JSX.Element {
  ...
  const numFirstPartyOpcodes = _.filter(
    data.decodetree.matches,
    (x) => x.match <= 0b011111,
  ).length
  const numUnifiedExtOpcodes = _.filter(
    data.decodetree.matches,
    (x) => x.match >= 0b100000 && x.match <= 0b101111,
  ).length
  const numInsnFormats = _.uniq(_.map(data.insns, (x) => x.format.repr)).length
  const allocationRatio =
    (augmentedDecodeTree.numUsedInsnWords /
      augmentedDecodeTree.numTotalInsnWords) *
    100
  // HACK: we know all insns are 1st-party so far, so no need to really filter anything
  const firstPartyAllocationRatio =
    (augmentedDecodeTree.numUsedInsnWords / 0x80000000) * 100
    ...
    <>
      <Row gutter={16}>
        <Col xs={24} xl={3}>
          <Row>
            ...
            <Col xs={12} xl={24}>
              <Statistic
                title="指令格式数"
                value={numInsnFormats}
                style={{ marginTop: screens.xl ? 16 : 0 }}
              />
            </Col>
          </Row>
          <Row>
          ...
          <Row>
            ...
            <Col xs={12} xl={24}>
              <Statistic
                title="已分配第三方统一扩展主操作码"
                value={numUnifiedExtOpcodes}
                suffix="/ 16"
                style={{ marginTop: 16 }}
              />
            </Col>
          ...
          <Row>
            ...
            <Col xs={12} xl={24}>
              <Statistic
                title="已分配第一方编码空间"
                value={firstPartyAllocationRatio.toFixed(2)}
                suffix="%"
                style={{ marginTop: 16 }}
              />
            </Col>
        ...
        <Col xs={24} xl={9} style={{ marginTop: screens.xl ? 0 : 16 }}>
          <DecodeTreeNodeDetail
            data={augmentedDataMap}
