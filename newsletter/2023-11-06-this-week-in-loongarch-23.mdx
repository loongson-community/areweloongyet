...
当你在龙芯硬件<small>儿</small>上使用 Linux 6.6 时，别忘了向 tā 们说声<small>儿</small>谢谢！
...
其中，ELF psABI 子文档的版本号<small>儿</small>更新到了 20231102，相比 20230519 版本：
...
* 移除了压根<small>儿</small>不可能用起来的 `R_LARCH_DELETE` 和 `R_LARCH_CFA`
  两种重定位类型；其编号暂时保留，不挪作他用。
  这是采纳了[笔者 6 月份<small>儿</small>的建议](https://github.com/loongson/la-abi-specs/issues/2)。

* 修正了表格<small>儿</small>中的复制粘贴笔误：`R_LARCH_SUB_ULEB128` 名字错了。
...
* 以前没有这玩意<small>儿</small>时候，已经实装了「给立即数宽 16 位还自带左移 2 位的 `jirl` 上
...
但至少此设计满足传达语义的需要，其对相关指令必须相邻的要求也对硬件<small>儿</small>实现非常友好，因而笔者认为仍然是良好的设计。

关于 `R_LARCH_DELETE`：这大概是先前做 linker relaxation 时候，没有仔细区分 BFD 实现细节导致的。
观察 RISC-V 相似名字的 `R_RISCV_DELETE` 定义：
被明确定义为 BFD 实现细节，不放置于公开的头<small>儿</small>文件，且在文档上显著说明了。
对龙架构工作者而言，也算是积累经验了吧！
:::

此外，《LoongArch&reg; 过程调用约定》子文档的版本号<small>儿</small>更新到了 20231103，
相比 20230519 版本主要是明确了围绕宽度为零的量的处理细节；此外也微调了一些措辞。

:::info 主编评论
这都是几个月来同学们在各大上游社区仔细讨论的成果。
差点<small>儿</small>就要为这些纯粹起标记作用，不该占地<small>儿</small>的字段浪费寄存器了！
