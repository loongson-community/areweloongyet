...
Song Gao 为龙架构 KVM [实现了](https://lore.kernel.org/loongarch/45b93b94-fceb-c41e-9b3a-c2b3187eecae@loongson.cn/T/#t)直接消息信号中断控制器（DMSINTC）支持，主要目的是使 irqfd 机制能够将消息信号中断（MSI）直接注入到指定的目标 vCPU。使用 `netperf` 进行 `UDP_RR` 测试显示，启用 DMSINTC 后网络吞吐性能提升约 6.3% 。
...
Binbin Zhou 为 Loongson-2K0300 SoC [引入了](https://lore.kernel.org/loongarch/ad6_-5NQv_Q6vxAE@zenone.zhora.eu/T/#t)新的 I2C 控制器驱动（`i2c-ls2x-v2`），支持标准与快速模式、主模式传输和中断驱动。

#### 其他内核功能 {/* #other-news */}

Youling Tang 为龙架构[实现了](https://lore.kernel.org/loongarch/20260405021227.532599-1-youling.tang@linux.dev/T/#t) `flush_icache_all()` 函数并实现 jump_label 批量化更新，将多个指令补丁的 ibar（补丁描述为 I-cache 并不准确）操作合并为一次。
...
:::info[栈哨兵标志（Stack Canary）命名由来]

`canary`（金丝雀）的命名借自煤矿中用作毒气预警的金丝雀：stack canary 是在函数栈帧中放置的随机值，函数返回前若该值被修改则说明发生了栈溢出攻击，程序立即停止以防恶意代码执行。此次修正使龙架构能够统一实现栈哨兵标志的初始化。

:::

Huacai Chen [实现了](https://lore.kernel.org/loongarch/20260409122348.2438400-1-chenhuacai@loongson.cn/T/#u) `cpu_show_spectre_v1()` 函数，使得用户可通过 `/sys/devices/system/cpu/vulnerabilities/spectre_v1` 查看 Spectre V1 漏洞状态和缓解措施。
...
Haiyong Sun 和 WANG Rui 为 perf 构建[启用了](https://lore.kernel.org/loongarch/CAAhV-H4q-LUUVLQEXtpm2dARfqZGfKk6Tz=f1_2Uo_ixAjebOg@mail.gmail.com/T/#t)龙架构的 Clang 和 Rust 目标标志，支持 Clang 编译和 Rust 交叉编译。
...
Lulu Cheng [修复了](https://gcc.gnu.org/pipermail/gcc-patches/2026-April/712935.html)因成本计算不完整而导致的错误指令序列生成问题。由于龙架构后端未为 `ORDERED` 操作定义成本，导致 `UNORDERED` 的成本被评估为大于 `ORDERED`，触发了编译器将 `UNORDERED` 转换为 `ORDERED` 的“反转”优化，进而使 `lasx-vcond-2.c` 和 `lsx-vcond-2.c` 测试失败。
...
wangleiat [修复了](https://github.com/llvm/llvm-project/pull/192033)使用 `--emit-relocs` 选项时 LLD 错误丢弃 `R_LARCH_MARK_LA` 重定位项的问题。`R_LARCH_MARK_LA` 是一种标记重定位，用于在二进制文件中标记 `la.abs` 指令序列，主要服务于 EDK II 构建流程。
